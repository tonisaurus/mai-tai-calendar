...
def parse_standings(raw: list, team: str) -> Optional[Standings]:
    ...
    for division in raw:
        ...
        standings = Standings(division=division.get("divisionName", ""), rows=sorted(rows, key=lambda r: r.rank))
...
def standings_table(standings: Standings) -> list[str]:
    ...
    lines = [f"Standings - {standings.division.strip()}"]
    for row in standings.rows:
        lines.append(f"{row.rank}. {row.team} {row.record}, {row.points} pts, GD {row.goal_diff:+d}")
    return lines


def build_description(game: Game, team: str, featured: bool, standings: Optional[Standings]) -> str:
    lines: list[str] = []
    home, away = clean_name(game.home.name), clean_name(game.away.name)
    ...
    elif game.is_final and game.home.score is not None and game.away.score is not None:
        outcome = {"W": f"{team} won", "L": f"{team} lost", "D": "Draw"}.get(result_letter(game, team), "Final")
        extras = ", ".join(t for t in ("OT" if game.overtime else "", "shootout" if game.shootout else "") if t)
        lines.append(f"Final: {home} {game.home.score} - {game.away.score} {away} ({outcome}{', ' + extras if extras else ''})")
    else:
...
def build(config: dict, games: list[Game], standings: Optional[Standings], state: dict, now: datetime, location: str) -> tuple[str, dict]:
    ...
    for game in ours:
        ...
        description = build_description(game, team, is_featured, standings)
        uid = event_uid(game)
